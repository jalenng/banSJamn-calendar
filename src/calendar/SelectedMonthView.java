...
public class SelectedMonthView extends JPanel implements CalendarView {
	...
	public void display() {
		...
		// Create header and body of the panel. Header has month, year, and buttons. Body has the days
		...
		JLabel titleLabel = new JLabel(
				visibleDate.getMonth().name() + " " + visibleDate.getYear(), 
		...
		for (int i = 0; i < 6; i++) {
			for (int j = 0; j < 7; j++) {
				...
				//else if we haven't reached the end of the month
					...
					LocalDate buttonRepresentedDate = 
							visibleDate.withDayOfMonth(Integer.parseInt(dateButton.getText()));
