...
public class Event implements Comparable<Event>{
	...
	/**
	 * Checks to see if an event occurs on a given date.
	 * Useful for recurring events, but also works on nonrecurring events.
	 * @param	Date to check if event occurs on
	 * @return	true if event occurs on the given date, false otherwise
	 */
	public boolean occursOn(LocalDate date) {
		if (!this.isRecursive && this.date.equals(date)) return true;
	...
	public boolean conflictsWith(Event that) { //"that" will never be recurring
		return this.occursOn(that.getDate()) 
				&& this.timeInterval.conflictsWith(that.timeInterval);
	}
