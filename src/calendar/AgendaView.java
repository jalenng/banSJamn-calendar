...
public class AgendaView extends JPanel implements CalendarView {
	...
	/**
	 * Sets whether it is a new interval.
	 * 
	 * @param n
	 */
		...
		submitButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					startDate = LocalDate.of(Integer.parseInt(startArray[2]), Integer.parseInt(startArray[0]),
							Integer.parseInt(startArray[1]));

					endDate = LocalDate.of(Integer.parseInt(endArray[2]), Integer.parseInt(endArray[0]),
							Integer.parseInt(endArray[1]));

					setNewInterval(false);
					displayAgenda();
				} catch (DateTimeException exception) {
					JOptionPane.showMessageDialog(null, "Invalid date.", "Agenda View", JOptionPane.ERROR_MESSAGE);
				} catch (Exception exception2) {
					JOptionPane.showMessageDialog(null, "Invalid date format.", "Agenda View",
